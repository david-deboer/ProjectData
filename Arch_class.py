...
class Data:
    def __init__(self,dirName='Architecture',verbosity=True):
        ...
        print 'Architecture path = '+self.path
    ...
    def readData(self,verbosity=None):
        ...
        file2open = os.path.join(self.path,self.archFile)
        fp = open(file2open,'r')
    ...
    def showData(self,howsort='value',handleSpace=25,descSpace=50,short=True):
        """the arguments are to make it consistent with showData for milestones/requirements/etc  -- maybe I'll use them someday"""
        inLevs = True
        n = 1
        print '\nCOMPONENTS:'
        while inLevs:
...
################CUT-AND-PASTE COMPONENTS INTO SYSTEM
    ...
    def _readElement(self,reading,line,verbosity):
        ...
        if len(identry)==1:
            ...
            print 'invalid entry:  '+str(identry),
            idpath = os.path.join(idpath,self.cn)
